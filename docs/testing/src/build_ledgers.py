...
B1 = [
 ...
 {"art": "IV", "title": "At the Counter", "tag": "branches &middot; peds &middot; NUI", "why":
  ...
  "items": [
   {"id": "T1", "who": "a", "t": "Open the map.", "x": "<b>Expect:</b> seven bank blips &mdash; Valentine, Rhodes, Saint Denis, Blackwater, Tumbleweed, Armadillo and Annesburg."},
   ...
   {"id": "T2b", "who": "a", "t": "Run <code>banking_peds</code> in F8 while standing at the counter.", "x": "<b>Expect:</b> the branch reports <code>ped=yes</code> and <b>off mark under 0.5m</b>. All seven are now placed from surveyed clerk marks, so every one should read near zero &mdash; anything larger is a streaming or spawn fault, not a bad coordinate."},
   {"id": "T3", "who": "a", "gate": True, "t": "Stand at the counter and hold <b>[G]</b>.", "x": "<b>Expect:</b> the prompt appears, and holding it opens the bank. <b>If no prompt appears</b>, compare your coords to <code>teller</code> in <code>config/locations.lua</code> &mdash; every branch is now placed from a surveyed clerk mark, so a missing prompt points at the code rather than the coordinates."},
