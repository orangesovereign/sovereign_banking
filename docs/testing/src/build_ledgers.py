...
B1 = [
 {"art": "I", "title": "Before the Doors Open", "tag": "pre-flight", "why":
  ...
  "items": [
   {"id": "P1", "who": "b", "t": "Confirm <code>vorp_core</code>, <code>oxmysql</code> and <code>vorp_inventory</code> all start <b>before</b> <code>sov_bank</code> in your cfg.", "x": "<b>Expect:</b> sov_bank appears after all three in the start order."},
   {"id": "P2", "who": "b", "gate": True, "t": "Confirm the resource folder is named exactly <code>sov_bank</code> &mdash; not <code>sovereign_banking</code>.", "x": "<b>Expect:</b> the folder name matches, or every <code>exports.sov_bank:&hellip;</code> call from other scripts will fail to resolve."},
   ...
   {"id": "P4", "who": "b", "t": "Add <code>add_ace group.admin sovbank.admin allow</code> to <code>server.cfg</code> and reload it.", "x": "<b>Expect:</b> no error. This is what lets you open <code>/bankadmin</code> later."},
 ...
 {"art": "II", "title": "The Fresh Deploy", "tag": "boot &middot; schema &middot; seeding", "why":
  ...
  "items": [
   {"id": "S1", "who": "b", "gate": True, "t": "Start the resource and watch the console.", "x": "<b>Expect:</b> <code>Sovereign Bank v0.6.0 ready</code>, and <b>no red lines</b> mentioning sov_bank."},
   {"id": "S2", "who": "b", "t": "In the database: <code>SHOW TABLES LIKE 'sov_bank_%';</code>", "x": "<b>Expect:</b> 11 tables &mdash; accounts, access, transactions, loans, savings_accrual, sdb, bills, business_tax, seizures, liens, reserves (plus meta)."},
   {"id": "S3", "who": "b", "gate": True, "t": "Check the standing accounts:<pre class=\"sql\">SELECT id, account_number, owner_id FROM sov_bank_accounts\nWHERE owner_type='system';</pre>", "x": "<b>Expect:</b> exactly two rows &mdash; <code>1 / SVB-0000001 / SYS-GOV</code> and <code>2 / SVB-0000002 / SYS-INSURANCE</code>."},
   ...
   {"id": "S5", "who": "b", "t": "<code>SELECT * FROM sov_bank_reserves;</code>", "x": "<b>Expect:</b> one row per configured branch, each with <code>balance = cap</code>."},
   {"id": "S6", "who": "b", "gate": True, "t": "Log a character in, then run <code>sovbank account &lt;charid&gt;</code> in the console.", "x": "<b>Expect:</b> an account row whose number is <code>SVB-0001001</code> or higher. <b>If it is numbered below 1001</b>, the reserved-range ALTER did not apply &mdash; look for a SQL permission error at boot."},
 ...
 {"art": "III", "title": "The Money Invariants", "tag": "sovbanktest &middot; automated", "why":
  ...
  "items": [
   {"id": "M1", "who": "b", "gate": True, "t": "With a player online, run in the <b>server console</b>: <code>sovbanktest &lt;theirServerId&gt;</code>", "x": "<b>Expect:</b> a run of green PASS lines ending <code>suite done: N passed, 0 failed</code>. It creates throwaway accounts and deletes everything it made."},
 ...
 {"art": "IV", "title": "At the Counter", "tag": "branches &middot; peds &middot; NUI", "why":
  ...
  "items": [
   {"id": "T1", "who": "a", "t": "Open the map.", "x": "<b>Expect:</b> a bank blip at Valentine, Rhodes, Saint Denis and Blackwater."},
   ...
   {"id": "T3", "who": "a", "gate": True, "t": "Stand at the counter and hold <b>[G]</b>.", "x": "<b>Expect:</b> the prompt appears, and holding it opens the bank. <b>If no prompt appears</b>, compare your coords to <code>teller</code> in <code>config/locations.lua</code> &mdash; the shipped values are approximate."},
 ...
 {"art": "V", "title": "Deposits, Withdrawals &amp; the Wire", "tag": "the core loop", "why":
  ...
  "items": [
   ...
   {"id": "D6", "who": "b", "gate": True, "t": "After D5: <code>SELECT balance_money FROM sov_bank_accounts WHERE id=2;</code>", "x": "<b>Expect:</b> the insurance fund grew by <b>exactly the fee</b>. This is the money that pays for deposits being robbery-proof."},
 ...
 {"art": "VI", "title": "Accounts &amp; Shared Access", "tag": "hierarchy &middot; owner is untouchable", "why":
  ...
  "items": [
   ...
   {"id": "A8", "who": "b", "t": "After closing: check the ledger rows for that account id.", "x": "<b>Expect:</b> the account is gone from the list but <b>its history remains</b> in <code>sov_bank_transactions</code>. The ledger is append-only."},
 ]},
...
build('B1-FOUNDATION-LEDGER.html',
      ...
      'Boot, the money engine &amp; the counter — sov_bank v0.6.0',
...
B2 = [
 {"art": "I", "title": "Bills Issued", "tag": "invoice &middot; fine &middot; tax", "why":
  ...
  "items": [
   {"id": "B1", "who": "b", "t": "From the console or another resource:<pre class=\"sql\">exports.sov_bank:IssueInvoice('42', '77', 0, 12500, 'Cattle feed')</pre>", "x": "<b>Expect:</b> returns ok with a bill id, and a row in <code>sov_bank_bills</code> with <code>kind='invoice'</code>, status <code>pending</code>."},
   {"id": "B2", "who": "b", "t": "Now a government debt:<pre class=\"sql\">exports.sov_bank:IssueFine('77', 0, 5000, 'Disturbing the peace')</pre>", "x": "<b>Expect:</b> a bill with <code>kind='fine'</code>, <code>issuer_type='system'</code>."},
 ...
 {"art": "III", "title": "The Delinquency Ladder", "tag": "tiers 0&ndash;3 &middot; the hard rule", "why":
  ...
  "items": [
   {"id": "L1", "who": "b", "t": "Backdate a fine and let the sweep run (&le;15 min) or restart the resource:<pre class=\"sql\">UPDATE sov_bank_bills SET due_at = NOW() - INTERVAL 5 DAY WHERE id = ?;</pre>", "x": "<b>Expect:</b> status moves <code>pending</code> &rarr; <code>overdue</code>, and a late fee is added to <code>balance_remaining</code>."},
   ...
   {"id": "L4", "who": "b", "t": "Backdate a <b>fine</b> 20+ days with a balance over the arrestable threshold, then sweep.", "x": "<b>Expect:</b> status becomes <code>warrant</code> and the resource fires <code>sov_bank:server:warrantFiled</code>."},
 ...
 {"art": "IV", "title": "Society Funds &amp; Payroll", "tag": "job-gated &middot; atomic batch", "why":
  ...
  "items": [
   ...
   {"id": "Y8", "who": "b", "t": "<code>sovbank reconcile &lt;societyAccountId&gt;</code> after all of the above.", "x": "<b>Expect:</b> <code>ok: true</code>, no drift."},
 ]},
 {"art": "V", "title": "The Loan Book", "tag": "fixed-cost &middot; no compounding", "why":
  ...
  "items": [
   ...
   {"id": "N3", "who": "b", "t": "<code>sovbank loans</code> in the console.", "x": "<b>Expect:</b> the pending application is listed with borrower, principal and total due."},
   {"id": "N4", "who": "b", "gate": True, "t": "<code>sovbank loans approve &lt;id&gt;</code>", "x": "<b>Expect:</b> the principal is credited to the chosen account, status becomes <code>active</code>, and <code>due_by</code> is set."},
   ...
   {"id": "N8", "who": "b", "t": "File a fresh application and <code>sovbank loans deny &lt;id&gt;</code>.", "x": "<b>Expect:</b> status <code>denied</code>, and <b>nothing</b> was disbursed."},
...
build('B2-COMMERCE-LEDGER.html',
      ...
      'Bills, societies, loans, interest, gold &amp; the vault — sov_bank v0.6.0',
...
B3 = [
 {"art": "I", "title": "Business &amp; the Tax Ledger", "tag": "licence fee &middot; no sales tax", "why":
  ...
  "items": [
   {"id": "Z1", "who": "b", "t": "No purchase flow is wired yet, so register by hand:<pre class=\"sql\">exports.sov_bank:RegisterBusiness('valentine_gunsmith', '42', 200000,\n  { name = 'Valentine Gunsmith' })</pre>", "x": "<b>Expect:</b> a business account is created and a Tax Ledger row opens with the building price as its basis."},
   ...
   {"id": "Z3", "who": "b", "t": "<code>exports.sov_bank:AssessBusinessTax('valentine_gunsmith')</code>", "x": "<b>Expect:</b> owed rises by $500 and a <code>tax_assessed</code> row appears in the ledger."},
   ...
   {"id": "Z6", "who": "b", "gate": True, "t": "<code>sovbank reconcile &lt;businessAccountId&gt;</code>", "x": "<b>Expect:</b> <b>no drift.</b> Assessments are deliberately ledgered against no account (they are an obligation, not a movement) precisely so this holds."},
   {"id": "Z7", "who": "b", "t": "Backdate <code>next_due_at</code> in <code>sov_bank_business_tax</code> with a balance owed, then let the sweep run.", "x": "<b>Expect:</b> a <code>tax</code> bill opens against the <b>owner</b> and enters the collections pipeline as government debt."},
 ...
 {"art": "III", "title": "Lawful Seizure", "tag": "the guardrails", "why":
  ...
  "items": [
   {"id": "E1", "who": "b", "gate": True, "t": "<code>exports.sov_bank:IsSeizureAuthorized(collectorCharid, debtorWithNoDebt)</code>", "x": "<b>Expect:</b> <b>false.</b> No debt, no authority &mdash; this is what your restraint script must check before allowing a rope."},
   ...
   {"id": "E8", "who": "b", "t": "<code>SELECT * FROM sov_bank_seizures ORDER BY id DESC LIMIT 1;</code>", "x": "<b>Expect:</b> a row naming collector, debtor, the itemised JSON, assessed value, amount applied and surplus returned. Every seizure is disputable after the fact."},
 ...
 {"art": "IV", "title": "The Branch Reserve", "tag": "the guarantee", "why":
  ...
  "items": [
   {"id": "H1", "who": "b", "gate": True, "t": "<b>Before anything:</b> snapshot every balance.<pre class=\"sql\">SELECT SUM(balance_money) AS total FROM sov_bank_accounts;</pre>", "x": "<b>Expect:</b> a number. Write it down &mdash; H4 depends on it."},
   {"id": "H2", "who": "b", "t": "Rob the till:<pre class=\"sql\">exports.sov_bank:ClaimBranchReserve('valentine', 0,\n  { fraction = 0.6, looters = { '42' } })</pre>", "x": "<b>Expect:</b> returns <code>looted</code> and <code>remaining</code>, and the looter's <b>cash in hand</b> grows by the take."},
   {"id": "H3", "who": "b", "t": "<code>SELECT balance FROM sov_bank_reserves WHERE branch_id='valentine';</code>", "x": "<b>Expect:</b> down by <b>exactly</b> what was looted."},
   ...
   {"id": "H5", "who": "b", "t": "Find the heist row: <code>SELECT * FROM sov_bank_transactions WHERE category='heist' ORDER BY id DESC LIMIT 1;</code>", "x": "<b>Expect:</b> a row with <code>account_id</code> <b>NULL</b> and the branch named in the memo &mdash; the till is not an account, so it stays out of reconciliation."},
 ...
 {"art": "V", "title": "The Ledger Office", "tag": "/bankadmin &middot; permission", "why":
  ...
  "items": [
   ...
   {"id": "O10", "who": "a", "t": "Compare the branch till meters to <code>sov_bank_reserves</code>.", "x": "<b>Expect:</b> they match, including any till you emptied in Article IV."},
 ]},
 {"art": "VI", "title": "The Contract", "tag": "integration &middot; for the suite", "why":
  ...
  "items": [
   {"id": "K1", "who": "b", "gate": True, "t": "From <b>another resource</b>, call <code>exports.sov_bank:AddMoney(charid, 0, 2500, { reason='payroll' })</code>.", "x": "<b>Expect:</b> it pays, and the ledger row's <code>source_resource</code> names <b>that resource</b> &mdash; attribution is automatic."},
   ...
   {"id": "K4", "who": "b", "t": "Add a listener for <code>sov_bank:server:transactionCompleted</code> and move some money.", "x": "<b>Expect:</b> it fires with charid, account, amount, direction and category."},
   {"id": "K5", "who": "b", "t": "Listen for <code>sov_bank:server:warrantFiled</code> across a full delinquency run.", "x": "<b>Expect:</b> it fires for fines and taxes only &mdash; never for a private invoice."},
   {"id": "K6", "who": "b", "t": "<i>Only if you enabled it:</i> with <code>Config.Compat.enabled = true</code>, call <code>exports.sov_bank:addMoney(source, 0, 12.50)</code>.", "x": "<b>Expect:</b> $12.50 arrives &mdash; the shim takes a <b>source</b> and <b>display units</b>, unlike the native exports."},
 ]},
 {"art": "VII", "title": "The Books Balance", "tag": "closing audit", "why":
  ...
  "items": [
   {"id": "F1", "who": "b", "gate": True, "t": "<code>sovbankadmin reconcile 500</code> in the console.", "x": "<b>Expect:</b> <code>drifted: []</code>. Every account's ledger sum equals its stored balance, after a full day of abuse."},
   {"id": "F2", "who": "b", "t": "<code>sovbankadmin supply 7</code>", "x": "<b>Expect:</b> a sane picture &mdash; faucets and sinks by category, with nothing wildly lopsided you cannot explain."},
...
build('B3-ENFORCEMENT-LEDGER.html',
      ...
      'Business tax, collections, seizure, heists &amp; the ledger office — sov_bank v0.6.0',
